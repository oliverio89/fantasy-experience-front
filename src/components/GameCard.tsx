...
const GameCard: FunctionComponent<GameCardProps> = memo(
  ({
    ...
    const headerStyle: CSSProperties = useMemo(() => {
      return {
        backgroundImage:
          propBackgroundImage || (imageUrl ? `url('${imageUrl}')` : undefined),
      };
    ...
    const headerPadding =
      variant === "flex" || variant === "absolute"
        ? "pt-1.5 px-[21px] pb-[97.1px]"
        : "pt-[7px] px-6 pb-[133px]";

    const renderContent = () => (
      <>
        {/* Header with image and badge */}
        <div
          className={`self-stretch rounded-t-xl rounded-b-none flex flex-row items-start justify-end ${headerPadding} box-border bg-cover bg-[top] max-w-full z-[1] min-h-[240px]`}
          style={headerStyle}
        >
          {/* Badge container (sin sombra/cuadro) */}
          <div className="h-[120px] w-[120px] relative z-[2]">
            <img
              ...
              src={badgeIcon}
            />
            <div className="w-[86.8px] h-[42px] absolute !m-[0] right-[3.2px] bottom-[4.24px] [text-decoration:underline] font-extrabold flex items-center justify-center [transform:_rotate(-20.1deg)] [transform-origin:0_0] z-[3]">
              {badgeText}
            </div>
          </div>
        </div>
        ...
        <div
          className={`self-stretch flex flex-col items-start justify-start ${
            variant === "flex" || variant === "absolute"
              ? "gap-[46.6px]"
              : "gap-2"
          } max-w-full text-lg text-darkslategray ${
            variant === "flex" ? "mq450:gap-[23px]" : ""
          }`}
        >
          <div
            className={`self-stretch flex flex-col items-start justify-start ${
          ...
          >
            <h2
              className={`m-0 self-stretch relative ${
                variant === "simple" ? "text-13xl" : "text-11xl"
              } font-${
                variant === "simple" ? "bold" : "extrabold"
              } font-[inherit] text-black1 z-[1] mq450:text-lg mq450:leading-[14px] ${
                variant === "simple"
                  ? "mq1050:text-7xl mq1050:leading-[19px]"
                  : "mq1000:text-5xl mq1000:leading-[19px]"
              }`}
            >
              {title}
            </h2>
            ...
            <div
              className={`self-stretch flex flex-col items-start justify-start ${
                variant === "detailed" ? "pt-0 px-0 pb-[3px]" : ""
              } text-xl`}
            >
              ...
              <div
                className={`self-stretch ${
                  variant === "flex" || variant === "absolute"
                    ? "flex flex-col items-start justify-start gap-[10.1px]"
                    : ""
                } text-lg`}
              >
          ...
          {showButton && (
            <div
              className={`${
                variant === "simple" ? "self-stretch" : ""
              } flex flex-row items-start justify-start py-0 ${
                variant === "simple"
                  ? "pl-4 pr-6 mt-[-45px]"
                  : variant === "flex"
                  ? "pl-[7px] pr-[11px]"
            ...
            >
              <Button className="max-w-[320px]" label={buttonText} />
            </div>
          )}
        </div>
      </>
      ...
      return (
        ...
        >
          <div
            className={`${
              variant === "flex" ? "flex-1" : ""
            } shadow-[0px_6px_10px_4px_rgba(0,_0,_0,_0.15),_0px_2px_3px_rgba(0,_0,_0,_0.3)] rounded-xl flex flex-col items-start justify-start pt-0 px-0 ${cardPadding} box-border gap-[18.9px] max-w-full`}
            style={cardStyle}
          >
            {renderContent()}
          </div>
    ...
    return (
      ...
      >
        <div
          className={`self-stretch shadow-[0px_6px_10px_4px_rgba(0,_0,_0,_0.15),_0px_2px_3px_rgba(0,_0,_0,_0.3)] rounded-xl flex flex-col items-start justify-start pt-0 px-0 ${cardPadding} box-border ${
            variant === "detailed" ? "gap-3.5" : "gap-2"
          } max-w-full ${
            variant === "simple" ? "mq750:pb-[35px] mq750:box-border" : ""
          }`}
          style={cardStyle}
        >
          {renderContent()}
