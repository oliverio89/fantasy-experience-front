...
const NextGames: FunctionComponent<UpcomingCarouselType> = memo(
  ({ className = "" }) => {
    ...
    // Lista de tarjetas para el carrusel (duplicado para bucle continuo)
    const gameCards = [
      { imageUrl: "/cedericvandenbergheDPhytVHwunsplash-1@2x.png" },
      { imageUrl: "/konradkollerlctjo2d9-2cunsplash-1@2x.png" },
      { imageUrl: "/konradkollerlctjo2d9-2cunsplash-1@2x.png" },
      { imageUrl: "/konradkollerlctjo2d9-2cunsplash-1@2x.png" },
    ];

    // Crear duplicado para bucle infinito
    const extendedGameCards = [...gameCards, ...gameCards];
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {extendedGameCards.map((card, index) => (
              <GameCard
                key={index}
                propBackgroundImage="unset"
                cedericVandenbergheDPhytVHw={card.imageUrl}
                style={{ userSelect: "none", pointerEvents: "none" }} // Asegura que no se arrastre el fondo
              />
            ))}
          </div>
