...
export class ProfileService {
  ...
  /**
   * Obtiene lista de "Masters" (usuarios que han creado al menos una partida)
   * En un escenario ideal, haríamos un join o una query más compleja.
   * Por simplicidad y dado que Supabase JS client tiene límites en joins complejos,
   * podemos obtener perfiles y filtrar, o usar una RPC si fuera necesario.
   * Para este MVP: Obtenemos partidas únicas por master_id, y luego sus perfiles.
   */
  static async getMasters(): Promise<Profile[]> {
    try {
      // 1. Obtener IDs de masters con partidas creadas
      // Hacemos una query ligera a la tabla games
      ...
      // Extraer IDs únicos y limpiar nulos
      ...
      // 2. Obtener perfiles de esos masters
      ...
      const profiles = profilesData.map((p) => this.mapProfileFromDB(p));

      // 3. (Opcional) Calcular rating medio y total partidas si quisiéramos mostrarlos real
      // Por ahora devolvemos perfiles básicos enriquecidos
